...
test("une couverture exige un texte alternatif", () => {
  const result = completeResourceVersionSchema.safeParse({
    ...
    sourceMode: "TEXT",
    blocks: [{ type: "PARAGRAPH", text: "Contenu" }],
    coverMedia: "507f1f77bcf86cd799439011",
  });
  assert.equal(result.success, false);
});

test("un article importé accepte une couverture publique et des images intégrées", () => {
  const result = completeResourceVersionSchema.safeParse({
    ...base,
    format: "ARTICLE",
    sourceMode: "TEXT",
    coverUrl: "/images/articles/couverture.jpg",
    coverAlt: "Couverture de l’article",
    authorName: "Aina Rakotonjanahary",
    originalPublishedAt: "2024-11-22T00:00:00Z",
    blocks: [
      { type: "PARAGRAPH", text: "Contenu" },
      {
        type: "IMAGE",
        src: "/images/articles/illustration.jpg",
        alt: "Illustration",
      },
    ],
  });
