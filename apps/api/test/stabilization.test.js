...
const {
  cleanError,
  RETENTION_MS,
} = require("../src/services/emailDispatchLog.service");

test("les garde-fous distinguent les bases automatisée et staging", () => {
  const testUri =
    "mongodb+srv://user:pass@example.mongodb.net/plateforme_melanie_automated_test";
  const stagingUri =
    "mongodb+srv://user:pass@example.mongodb.net/plateforme_melanie_staging";
  ...
  assert.equal(
    assertStagingDatabase({ uri: stagingUri }),
    "plateforme_melanie_staging",
  );
});
...
test("les contrôles externes acceptent uniquement Resend et Cloudinary", () => {
  assert.equal(
    systemValidation.checksSchema.safeParse({
