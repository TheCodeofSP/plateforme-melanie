...
Accès public. Crée un compte membre. L’accès reste bloqué jusqu’à la validation de l’email et, pour une mineure, l’accord du responsable légal.
...
`guardianEmail` est obligatoire entre 15 et 17 ans, inutile à partir de 18 ans. L’inscription est refusée avant 15 ans. Le pseudonyme et l’email sont uniques. Le mot de passe doit comporter 8 à 128 caractères, avec majuscule, minuscule, chiffre et caractère spécial.
...
Réponse `200` : contient `accountActivated`. Pour une mineure, cette valeur reste `false` tant que l’autorisation parentale manque.
