...
const p = require("../../controllers/safePlace/post.controller");
...
router.get("/", validateQuery(v.postListSchema), p.list);
router.post(
  "/",
  createRateLimit({ max: 20, code: "SAFE_PLACE_POST_RATE_LIMIT" }),
  validateBody(v.postCreateSchema),
  p.create,
);
router.get("/:postId", validateParams(v.postIdSchema), p.detail);
router.patch(
  "/:postId",
  validateParams(v.postIdSchema),
  validateBody(v.postUpdateSchema),
  p.update,
);
router.delete("/:postId", validateParams(v.postIdSchema), p.remove);
router.post(
  "/:postId/correction",
  validateParams(v.postIdSchema),
  validateBody(v.postUpdateSchema),
...
router.get("/:postId/comments", validateParams(v.postIdSchema), c.list);
router.post(
  "/:postId/comments",
