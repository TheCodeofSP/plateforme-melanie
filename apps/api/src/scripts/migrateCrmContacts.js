...
async function run() {
  ...
  for (const participant of await QuizParticipant.find({ user: null })) {
    await crm.syncIdentity({ participant, source: "QUIZ_PUBLIC" });
    processed += 1;
  }
...
run().catch(async (error) => {
  console.error("❌ Migration CRM impossible :", error.message);
  await mongoose.connection.close();
  process.exit(1);
});
