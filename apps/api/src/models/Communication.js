...
const targetSchema = new mongoose.Schema(
  {
    roles: [{ type: String, enum: ["MEMBER", "INTERVENANT", "ADMIN"] }],
    spmProfiles: [
      {
        type: String,
        enum: [
    ...
    includeQuizContacts: { type: Boolean, default: false },
    quizFrom: Date,
    quizTo: Date,
    webinar: {
      type: mongoose.Schema.Types.ObjectId,
    ...
    webinarStatuses: [
      {
        type: String,
        enum: ["REGISTERED", "WAITLISTED", "PRESENT", "ABSENT", "CANCELLED"],
      },
    ],
    manualUsers: [{ type: mongoose.Schema.Types.ObjectId, ref: "User" }],
    manualQuizParticipants: [
      { type: mongoose.Schema.Types.ObjectId, ref: "QuizParticipant" },
    ],
    manualCrmContacts: [
      { type: mongoose.Schema.Types.ObjectId, ref: "CrmContact" },
    ],
  },
  { _id: false },
);
const schema = new mongoose.Schema(
  {
    ...
    internalTitle: { type: String, required: true, trim: true, maxlength: 180 },
    type: { type: String, enum: COMMUNICATION_TYPES, required: true },
    channel: { type: String, enum: ["EMAIL", "IN_APP"], required: true },
    subject: { type: String, trim: true, maxlength: 180, default: null },
    preheader: { type: String, trim: true, maxlength: 250, default: null },
    notificationTitle: {
      type: String,
      trim: true,
      maxlength: 180,
    ...
    replyTo: { type: String, trim: true, lowercase: true, default: null },
    preferenceCategory: {
      type: String,
      enum: [...MARKETING_CATEGORIES, null],
      default: null,
    },
    ...
    targeting: { type: targetSchema, default: () => ({}) },
    status: {
      type: String,
    ...
    frozenSnapshot: { type: mongoose.Schema.Types.Mixed, default: null },
    counters: {
      targeted: { type: Number, default: 0 },
      excludedConsent: { type: Number, default: 0 },
      excludedTechnical: { type: Number, default: 0 },
