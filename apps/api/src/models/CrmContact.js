...
const { CRM_STATUSES, CONTACT_SOURCES, TASK_PRIORITIES } = require("../config/dashboard.constants");
const sourceSchema = new mongoose.Schema({ type: { type: String, enum: CONTACT_SOURCES, required: true }, occurredAt: { type: Date, default: Date.now }, detail: { type: String, default: null, maxlength: 200 } }, { _id: false });
const consentSchema = new mongoose.Schema({ granted: { type: Boolean, default: false }, occurredAt: { type: Date, default: null }, source: { type: String, default: null, maxlength: 200 } }, { _id: false });
const schema = new mongoose.Schema({
  ...
  quizParticipant: { type: mongoose.Schema.Types.ObjectId, ref: "QuizParticipant", default: null, index: true },
  status: { type: String, enum: CRM_STATUSES, default: "NOUVEAU", index: true },
  ...
  mergedInto: { type: mongoose.Schema.Types.ObjectId, ref: "CrmContact", default: null, index: true },
  anonymizedAt: { type: Date, default: null, index: true },
  deletedAt: { type: Date, default: null, index: true },
}, { timestamps: true });
schema.index({ primaryEmail: 1 }, { unique: true, partialFilterExpression: { deletedAt: null } });
