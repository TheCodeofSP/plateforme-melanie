...
const quizAttemptSchema = new mongoose.Schema({
  ...
  userSnapshot: { type: mongoose.Schema.Types.ObjectId, ref: "User", default: null },
  quizVersion: { type: String, required: true },
  status: { type: String, enum: ["AWAITING_PROFILE_SELECTION", "COMPLETED"], required: true },
  answers: { type: [answerSchema], required: true },
  participantInfo: {
    age: { type: Number, required: true, min: 10, max: 100 },
    contraception: { type: String, enum: CONTRACEPTION_TYPES, required: true },
  },
  ...
  resultEmail: {
    status: { type: String, enum: ["NOT_READY", "PENDING", "SENT", "FAILED"], default: "NOT_READY" },
