  ...
  .superRefine((block, ctx) => {
    ...
    if (block.type === "IMAGE" && (!block.src || !block.alt)) {
      ctx.addIssue({
        code: "custom",
        path: ["src"],
        message: "Une image exige une source et un texte alternatif.",
      });
      return;
    }
    ...
    if (!isList && block.type !== "IMAGE" && !block.text)
      ctx.addIssue({
        code: "custom",
        path: ["text"],
...
const resourceVersionSchema = z
  .object({
    ...
    coverUrl: publicAssetUrl.nullable().optional(),
    ...
    authorName: z.string().trim().min(2).max(160).nullable().optional(),
    originalPublishedAt: z.iso.datetime().nullable().optional(),
  })
  .strict();
