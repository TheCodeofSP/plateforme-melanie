...
const envSchema = z.object({
  ...
  RESEND_QUIZ_SEGMENT_ID: z.preprocess((value) => value === "" ? undefined : value, z.string().uuid().optional()),
  RESEND_NEWSLETTER_SEGMENT_ID: z.preprocess((value) => value === "" ? undefined : value, z.string().uuid().optional()),
  RESEND_COMMERCIAL_SEGMENT_ID: z.preprocess((value) => value === "" ? undefined : value, z.string().uuid().optional()),
  RESEND_WEBHOOK_SECRET: z.preprocess((value) => value === "" ? undefined : value, z.string().min(20).optional()),
  ...
  CLOUDINARY_FOLDER_PREFIX: z.string().trim().default("plateforme-melanie/local"),
}).superRefine((value, context) => {
  if (value.APP_ENV !== "production") return;
  if (value.EMAIL_MODE === "capture") {
    context.addIssue({ code: "custom", path: ["EMAIL_MODE"], message: "EMAIL_MODE=capture est interdit en production." });
  }
  if (value.RESEND_FROM_EMAIL.endsWith("@resend.dev")) {
    context.addIssue({ code: "custom", path: ["RESEND_FROM_EMAIL"], message: "Un domaine Resend vérifié est obligatoire en production." });
  }
  if (!value.CRON_SECRET) {
    context.addIssue({ code: "custom", path: ["CRON_SECRET"], message: "CRON_SECRET est obligatoire en production." });
  }
  if (/local|test|staging/i.test(value.CLOUDINARY_FOLDER_PREFIX)) {
    context.addIssue({ code: "custom", path: ["CLOUDINARY_FOLDER_PREFIX"], message: "Le dossier Cloudinary de production est invalide." });
  }
});
