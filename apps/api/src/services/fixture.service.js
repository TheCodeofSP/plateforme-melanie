...
function fixtureTokenHash(key) {
  return crypto.createHash("sha256").update(`${PREFIX}${key}`).digest("hex");
}

async function upsertUser(key, data) {
  return User.findOneAndUpdate(
    { fixtureKey: `${PREFIX}${key}` },
    {
      $set: data,
      $setOnInsert: { fixtureKey: `${PREFIX}${key}` },
    },
...
async function seed() {
  ...
  for (let index = 0; index < profiles.length; index += 1) {
    ...
    members.push(
      await upsertUser(`member-${index + 1}`, {
        ...common,
  ...
  const memberWithoutQuiz = await upsertUser("member-no-quiz", {
    ...common,
    email: "member.noquiz.fixture@example.test",
    firstName: "Sansquiz",
    pseudonym: "sans-quiz-fixture",
    role: "MEMBER",
  });
  const suspended = await upsertUser("member-suspended", {
    ...common,
    email: "suspended.fixture@example.test",
    firstName: "Suspendue",
    pseudonym: "suspendue-fixture",
    role: "MEMBER",
    accountStatus: "SUSPENDED",
  });
  const pendingAdult = await upsertUser("member-pending-email", {
    ...common,
    email: "pending.email.fixture@example.test",
    firstName: "Activation",
    pseudonym: "activation-fixture",
    role: "MEMBER",
  ...
  const intervenant = await upsertUser("intervenant", {
    ...common,
    email: "intervenant.fixture@example.test",
    firstName: "Intervenante",
    pseudonym: "intervenante-fixture",
    role: "INTERVENANT",
  });
  const pendingIntervenant = await upsertUser("intervenant-pending", {
    ...common,
    email: "pending.intervenant.fixture@example.test",
    firstName: "Candidate",
    pseudonym: "candidate-fixture",
    role: "MEMBER",
  });
  const users = [
    admin,
  ...
  await SafePlaceSuspension.findOneAndUpdate(
    { fixtureKey: `${PREFIX}suspension` },
    {
      $set: {
        ...
        reason: "Suspension fictive de recette.",
        status: "ACTIVE",
      },
      $setOnInsert: { fixtureKey: `${PREFIX}suspension` },
    },
    { upsert: true },
  );
  ...
  const approvedApplication = await IntervenantApplication.findOneAndUpdate(
    ...
    {
      $set: {
        user: intervenant._id,
        professionalName: "Cabinet Fixture",
        profession: "Professionnelle de santé",
        specialties: ["Cycle menstruel"],
        presentation: "Profil fictif de recette.",
        motivations: "Recette de la plateforme.",
        status: "APPROVED",
        submittedAt: new Date(),
        decidedAt: new Date(),
        decidedBy: admin._id,
      },
      $setOnInsert: { fixtureKey: `${PREFIX}application-approved` },
    },
  ...
  await IntervenantApplication.findOneAndUpdate(
    ...
    {
      $set: {
        user: pendingIntervenant._id,
        professionalName: "Candidate Fixture",
        profession: "Intervenante",
        specialties: ["Bien-être"],
        presentation: "Candidature fictive.",
        motivations: "Recette de validation.",
        status: "PENDING",
        submittedAt: new Date(),
      },
      $setOnInsert: { fixtureKey: `${PREFIX}application-pending` },
    },
    { upsert: true },
  );
  ...
  await ProfessionalProfile.findOneAndUpdate(
    { fixtureKey: `${PREFIX}professional-profile` },
    {
      $set: {
        user: intervenant._id,
        sourceApplication: approvedApplication._id,
        draftVersion: profileVersion,
        publishedVersion: profileVersion,
      ...
      $setOnInsert: { fixtureKey: `${PREFIX}professional-profile` },
    },
    { upsert: true },
  );
  ...
  for (let index = 0; index < profiles.length; index += 1) {
    ...
    const participant = await QuizParticipant.findOneAndUpdate(
      { fixtureKey: `${PREFIX}quiz-participant-${index + 1}` },
      {
        ...
        $setOnInsert: { fixtureKey: `${PREFIX}quiz-participant-${index + 1}` },
      },
      { upsert: true, new: true },
    );
    ...
    const attempt = await QuizAttempt.findOneAndUpdate(
      { fixtureKey: `${PREFIX}quiz-attempt-${index + 1}` },
      {
        $set: {
          ...
          resultEmail: { status: "SENT", sentAt: new Date(), attempts: 1 },
        },
        $setOnInsert: { fixtureKey: `${PREFIX}quiz-attempt-${index + 1}` },
      },
      { upsert: true, new: true },
    );
      ...
      await QuizConsentRecord.findOneAndUpdate(
        { attempt: attempt._id, type },
        {
          $setOnInsert: {
            ...
            type,
            granted: true,
            textVersion: DOCUMENT_VERSIONS[type],
            acceptedAt: new Date(),
          },
        },
        { upsert: true },
  ...
  const category = await SafePlaceCategory.findOneAndUpdate(
    { fixtureKey: `${PREFIX}safe-place-category` },
    {
      $set: {
        ...
        slug: "categorie-recette-stabilisation",
        description: "Catégorie fictive pour les parcours de recette.",
        displayOrder: 999,
        status: "ACTIVE",
      },
      $setOnInsert: { fixtureKey: `${PREFIX}safe-place-category` },
    },
    { upsert: true, new: true },
  );
  ...
  await Resource.findOneAndUpdate(
    { fixtureKey: `${PREFIX}resource` },
    {
      $set: {
        owner: admin._id,
        authorRole: "ADMIN",
        slug: "ressource-recette-stabilisation",
        publicationStatus: "PUBLISHED",
        reviewStatus: "APPROVED",
        publishedVersion: {
          title: "Ressource fictive de recette",
          description: "Contenu fictif pour tester le frontend.",
          format: "ARTICLE",
      ...
      $setOnInsert: { fixtureKey: `${PREFIX}resource` },
    },
    { upsert: true },
  );
  ...
  const webinar = await Webinar.findOneAndUpdate(
    ...
    {
      $set: {
        title: "Webinaire fictif de recette",
        slug: "webinaire-recette-stabilisation",
        shortDescription: "Webinaire destiné aux tests.",
        description: "Description fictive du webinaire de recette.",
        host: admin._id,
        recommendedProfiles: ["BOULE_DE_NERFS"],
        status: "PUBLISHED",
        publishedAt: new Date(),
      },
      $setOnInsert: { fixtureKey: `${PREFIX}webinar` },
    },
    { upsert: true, new: true },
  );
  await WebinarSession.findOneAndUpdate(
    { fixtureKey: `${PREFIX}webinar-session` },
    {
      ...
      $setOnInsert: { fixtureKey: `${PREFIX}webinar-session` },
    },
    { upsert: true },
  );
  ...
  await Communication.findOneAndUpdate(
    ...
    {
      $set: {
        internalTitle: "Communication fictive de recette",
        type: "PLATFORM_NEWS",
        channel: "IN_APP",
        notificationTitle: "Actualité de recette",
        notificationMessage: "Communication fictive destinée au frontend.",
        preferenceCategory: "PLATFORM_NEWS",
        targeting: { roles: ["MEMBER"] },
        status: "DRAFT",
        createdBy: admin._id,
      },
      $setOnInsert: { fixtureKey: `${PREFIX}communication` },
    },
    { upsert: true },
  );
  await Notification.findOneAndUpdate(
    ...
    {
      $set: {
        recipient: members[0]._id,
        nature: "PERSONAL",
        category: "RESOURCES",
        type: "RESOURCE_PUBLISHED",
        title: "Notification fictive",
        message: "Une ressource fictive est disponible.",
        actionPath: "/resources/ressource-recette-stabilisation",
      },
      $setOnInsert: { fixtureKey: `${PREFIX}notification` },
    },
    { upsert: true },
  );
  for (let index = 0; index < profiles.length; index += 1) {
    await CrmContact.findOneAndUpdate(
      ...
      {
        $set: {
          primaryEmail: `prospect${index + 1}.fixture@example.test`,
          firstName: `Prospect${index + 1}`,
          lastName: "Fixture",
          status: "NOUVEAU",
          sources: [{ type: "QUIZ_PUBLIC" }],
          marketingConsent: {
            granted: true,
            occurredAt: new Date(),
            source: "FIXTURE",
          },
          currentSpmProfile: profiles[index][0],
          currentContraception: profiles[index][1],
        },
        $setOnInsert: { fixtureKey: `${PREFIX}crm-${index + 1}` },
      },
      { upsert: true },
    );
  }
...
async function reset() {
  const users = await User.find({ fixtureKey: new RegExp(`^${PREFIX}`) })
    .select("_id +fixtureKey")
    .lean();
  const userIds = users.map((user) => user._id);
  await Promise.all([
    ConsentRecord.deleteMany({ user: { $in: userIds } }),
    Session.deleteMany({ user: { $in: userIds } }),
    AccountToken.deleteMany({ user: { $in: userIds } }),
    ParentalAuthorization.deleteMany({ user: { $in: userIds } }),
    EmailChangeRequest.deleteMany({ user: { $in: userIds } }),
    AdminActionLog.deleteMany({
      $or: [{ admin: { $in: userIds } }, { targetUser: { $in: userIds } }],
    }),
    QuizConsentRecord.deleteMany({
      participant: {
        $in: await QuizParticipant.find({
          fixtureKey: new RegExp(`^${PREFIX}`),
        }).distinct("_id"),
      },
    }),
