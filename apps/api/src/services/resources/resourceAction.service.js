...
async function createActionRequest(resourceId, user, data) {
  ...
  if (user.role !== "INTERVENANT" || resource.authorRole !== "INTERVENANT")
    throw createResourceError(
      "Action réservée aux intervenantes.",
      "FORBIDDEN",
      403,
    );
  if (!resource.publishedVersion || resource.publicationStatus === "ARCHIVED")
    throw createResourceError(
  ...
  if (
    data.type === "UNPUBLISH" &&
    !["PUBLISHED", "SCHEDULED"].includes(resource.publicationStatus)
  )
    throw createResourceError(
      "Cette ressource est déjà dépubliée.",
      "ACTION_NOT_AVAILABLE",
      409,
    );
  if (
    await ResourceActionRequest.exists({
      resource: resourceId,
      type: data.type,
      status: "PENDING",
    })
    ...
    throw createResourceError(
      ...
      "ACTION_REQUEST_EXISTS",
      409,
    );
  const request = await ResourceActionRequest.create({
    resource: resourceId,
...
async function listAdminActionRequests(query = {}) {
  ...
  const [requests, total] = await Promise.all([
    ResourceActionRequest.find(filter)
      .populate("resource", "slug publishedVersion.title publicationStatus")
      .populate("requestedBy", "pseudonym")
...
async function listMyActionRequests(resourceId, user) {
  ...
  return ResourceActionRequest.find({
    resource: resourceId,
    requestedBy: user._id,
  })
    .sort({ createdAt: -1 })
...
async function decideActionRequest(requestId, admin, approved, comment) {
  const request = await ResourceActionRequest.findOne({
    _id: requestId,
    status: "PENDING",
  });
  if (!request)
  ...
  if (approved) {
    resource.publicationStatus =
      request.type === "ARCHIVE" ? "ARCHIVED" : "UNPUBLISHED";
    if (request.type === "ARCHIVE") resource.archivedAt = new Date();
    else resource.unpublishedAt = new Date();
    await resource.save();
  }
  await request.save();
  await log(
    resource._id,
    admin,
    `${request.type}_${approved ? "APPROVED" : "REJECTED"}`,
    comment,
  );
  await notifyOwner(resource, {
    type:
      request.type === "ARCHIVE" ? "RESOURCE_ARCHIVED" : "RESOURCE_UNPUBLISHED",
    subject: `Demande de ${request.type === "ARCHIVE" ? "classement" : "dépublication"}`,
    title: resource.publishedVersion?.title || "Ressource",
    message: `Ta demande a été ${approved ? "acceptée" : "refusée"}.`,
    comment,
  });
  return request;
}
...
async function republish(resourceId, admin) {
  ...
  if (!resource)
    throw createResourceError(
      "Cette ressource ne peut pas être remise en ligne.",
      "RESOURCE_NOT_UNPUBLISHED",
      409,
    );
  resource.publicationStatus = "PUBLISHED";
  resource.unpublishedAt = null;
  resource.lastPublishedAt = new Date();
  await resource.save();
  await log(resource._id, admin, "REPUBLISHED");
  return resource;
}
...
async function adminSetPublicationStatus(resourceId, admin, status, reason) {
  ...
  await resource.save();
  await log(resource._id, admin, status, reason);
  return resource;
}
...
async function setVisibility(resourceId, admin, visibility, reason) {
  ...
  if (!resource?.publishedVersion || resource.publicationStatus === "ARCHIVED")
    throw createResourceError(
      "La visibilité ne peut pas être modifiée.",
      "ACTION_NOT_AVAILABLE",
      409,
    );
  const previous = resource.finalVisibility;
  resource.finalVisibility = visibility;
  await resource.save();
  const ids = [
  ...
  if (ids.length)
    await MediaAsset.updateMany({ _id: { $in: ids } }, { visibility });
  await log(resource._id, admin, "VISIBILITY_CHANGED", reason, {
    previous,
    visibility,
  });
  return resource;
...
module.exports = {
  createActionRequest,
  listMyActionRequests,
  listAdminActionRequests,
  decideActionRequest,
  republish,
  adminSetPublicationStatus,
  setVisibility,
};
