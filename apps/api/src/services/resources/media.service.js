...
function cloudinaryTypes(purpose) {
  if (
    ["COVER", "PROFILE_PHOTO", "WEBINAR_IMAGE", "COMMUNICATION_IMAGE"].includes(
      purpose,
    )
  )
...
async function authorizeUpload(user, data) {
  ...
  if (data.resourceId) {
    ...
    if (
      ...
      throw createResourceError(
        "Cette ressource ne t’appartient pas.",
        "RESOURCE_FORBIDDEN",
        403,
      );
  }
  ...
  const folder =
    data.purpose === "PROFILE_PHOTO"
      ? "professionals"
      : data.purpose === "WEBINAR_IMAGE"
        ? "webinars"
        : data.purpose === "COMMUNICATION_IMAGE"
          ? "communications"
          : "resources";
...
async function confirmUpload(user, data) {
  ...
  if (!media)
    throw createResourceError(
      "Ce média ne peut pas être confirmé.",
  ...
  if (
    !signaturesMatch(data.signature, expectedSignature) ||
    data.publicId !== media.storageKey ||
    data.resourceType !== media.resourceType
  ) {
    throw createResourceError(
      "La réponse Cloudinary n’est pas authentique ou ne correspond pas au média attendu.",
      "MEDIA_MISMATCH",
      400,
    );
  }
  if (data.bytes > media.size)
    throw createResourceError(
...
async function findActiveResource(media) {
  return Resource.findOne({
    ...
    $or: [
      { "publishedVersion.coverMedia": media._id },
      { "publishedVersion.media": media._id },
      { "publishedVersion.pdf": media._id },
    ],
  });
}
...
async function accessMedia(mediaId, user, disposition = "inline") {
  ...
  if (media.status === "ACTIVE") {
    ...
    if (!activeResource) {
      const isPublicProfilePhoto =
        media.purpose === "PROFILE_PHOTO" &&
        (await ProfessionalProfile.exists({
          publicationStatus: "PUBLISHED",
          isActive: true,
          "publishedVersion.photo": media._id,
        }));
      const isPublicWebinarImage =
        media.purpose === "WEBINAR_IMAGE" &&
        (await Webinar.exists({
          image: media._id,
          status: { $in: ["PUBLISHED", "COMPLETED"] },
        }));
      const isCommunicationImage =
        media.purpose === "COMMUNICATION_IMAGE" &&
        (await Communication.exists({
          _id: media.communication,
      ...
      if (
        ...
        !isPublicWebinarImage &&
        !isCommunicationImage &&
        !previewAllowed
      )
        throw createResourceError(
          "Ce média n’est pas disponible.",
      ...
      activeResource.finalVisibility === "MEMBERS_ONLY" &&
      media.purpose !== "COVER" &&
      !user
    ) {
      throw createResourceError(
  ...
  if (media.deliveryType === "upload") {
    url = cloudinary.url(media.storageKey, {
      resource_type: media.resourceType,
      type: media.deliveryType,
      version: media.providerVersion,
      format: media.providerFormat,
      secure: true,
    });
  } else {
...
async function destroyAsset(media) {
  configureCloudinary();
  return cloudinary.uploader.destroy(media.storageKey, {
    resource_type: media.resourceType,
    type: media.deliveryType,
    invalidate: true,
  });
...
async function deleteMedia(mediaId, user) {
  ...
  const inUse = await Resource.exists({
    $or: [
      { "publishedVersion.coverMedia": media._id },
      { "publishedVersion.media": media._id },
      { "publishedVersion.pdf": media._id },
      { "workingVersion.coverMedia": media._id },
      { "workingVersion.media": media._id },
      { "workingVersion.pdf": media._id },
    ],
  });
  const inProfile =
    media.purpose === "PROFILE_PHOTO" &&
    (await ProfessionalProfile.exists({
      $or: [
        { "draftVersion.photo": media._id },
        { "publishedVersion.photo": media._id },
      ],
    }));
  const inWebinar =
    media.purpose === "WEBINAR_IMAGE" &&
    (await Webinar.exists({ image: media._id }));
  const inCommunication =
    media.purpose === "COMMUNICATION_IMAGE" &&
    (await Communication.exists({
      $or: [
        { mainImage: media._id },
        { "blocks.image": media._id },
        { "frozenSnapshot.mainImage": media._id },
        { "frozenSnapshot.blocks.image": media._id },
      ],
    }));
  if (inUse || inProfile || inWebinar || inCommunication)
    throw createResourceError(
      "Ce média est encore utilisé.",
      "MEDIA_IN_USE",
      409,
    );
  await destroyAsset(media);
