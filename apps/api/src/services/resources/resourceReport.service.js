...
async function createReport(user, data) {
  ...
  if (!resource)
    throw createResourceError(
      "Cette ressource ne peut pas être signalée.",
  ...
  if (data.targetType === "COMMENT") {
    ...
    if (!comment)
      throw createResourceError(
        "Ce commentaire n’est pas disponible.",
        "COMMENT_NOT_FOUND",
        404,
      );
  }
  const duplicate = await ResourceReport.exists({
    reporter: user._id,
...
async function listReports(query = {}) {
  ...
  const [reports, total] = await Promise.all([
    ResourceReport.find(filter)
      ...
      .limit(limit)
      .lean(),
    ResourceReport.countDocuments(filter),
  ]);
  return {
    reports,
...
async function resolveReport(reportId, admin, data) {
  ...
  if (data.resolution === "COMMENT_REMOVED") {
    if (!report.comment)
      throw createResourceError(
        "Ce signalement ne concerne pas un commentaire.",
        "INVALID_RESOLUTION",
        400,
      );
    ...
    await requestChanges(
      ...
      admin,
      data.comment ||
        "Une correction est demandée à la suite d’un signalement.",
    );
  } else if (
    ["RESOURCE_UNPUBLISHED", "RESOURCE_ARCHIVED"].includes(data.resolution)
  ) {
    const status =
      data.resolution === "RESOURCE_ARCHIVED" ? "ARCHIVED" : "UNPUBLISHED";
    const update = {
  ...
  report.adminComment = data.comment;
