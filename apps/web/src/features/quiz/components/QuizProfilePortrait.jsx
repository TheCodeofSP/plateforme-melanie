...
export default function QuizProfilePortrait({ profile, compact = false }) {
  ...
  return (
    <article className={`quiz-portrait quiz-portrait--${visual.className || "default"} ${compact ? "quiz-portrait--compact" : ""}`}>
      <div className="quiz-portrait__marker" aria-hidden="true">{visual.symbol || "✦"}</div>
      ...
      {!compact && profile.body?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
    </article>
  );
}
