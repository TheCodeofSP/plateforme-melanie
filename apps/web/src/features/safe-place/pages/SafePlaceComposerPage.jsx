...
export default function SafePlaceComposerPage() {
  ...
  async function upload(event) {
    ...
    if (invalidFile) {
      setError(
        new Error("Chaque image doit être au format JPEG, PNG ou WebP et peser 5 Mo maximum."),
      );
      event.target.value = "";
      return;
    }
    ...
    try {
      ...
      for (const file of files) {
        const media = await uploadSafePlaceImage(file);
        added.push({ media: media._id, alt: "" });
      }
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (postId)
        post = location.state?.correction
          ? await submitPostCorrection(postId, editableFields)
          : await updatePost(postId, editableFields);
  ...
  return (
    <SafePlaceShell compact>
      <SEO title={postId ? "Modifier la discussion" : "Ouvrir une discussion"} noIndex />
      <p className="section-eyebrow">Déposer une parole</p>
      <h1>{postId ? "Faire évoluer ta discussion" : "Ouvrir une discussion"}</h1>
      <p className="clearing-privacy">{safePlaceContent.composer.privacy}</p>
      {error && <FormErrorSummary error={error} />}
      <form className="clearing-composer" onSubmit={submit}>
        <label>
          <span>Catégorie</span>
          <select
          ...
          >
            ...
            {categories
              .filter((category) => category.allowNewPosts && (isAdmin || !category.adminOnly))
              .map((category) => (
        ...
        <label>
          <span>Ce que tu souhaites déposer</span>
          <textarea
            value={form.content}
            minLength="10"
            maxLength="10000"
          ...
          />
          <small>
            {form.content.length}/10000 · {safePlaceContent.composer.mention}
          </small>
        </label>
        <fieldset>
          <legend>Liens facultatifs ({form.links.length}/3)</legend>
          ...
          {form.links.length < 3 && (
            <button
              type="button"
              onClick={() => field("links", [...form.links, { label: "", url: "" }])}
            >
              Ajouter un lien
            </button>
          )}
        ...
        <fieldset>
          <legend>Images facultatives ({form.images.length}/3)</legend>
          {form.images.map((image, index) => (
            <label key={image.media}>
              <span>Texte alternatif de l’image {index + 1}</span>
              <input
                value={image.alt}
                minLength="2"
                maxLength="300"
        ...
        {isAdmin && (
          <fieldset>
            <legend>Réglages de l’annonce</legend>
            <label className="clearing-check">
              <input
                type="checkbox"
                checked={form.allowComments}
                onChange={(event) => field("allowComments", event.target.checked)}
              />
            ...
            <label className="clearing-check">
              <input
                ...
                onChange={(event) => field("allowReactions", event.target.checked)}
              />
            ...
            {!postId && (
              <label className="clearing-check">
                <input
                  ...
                  onChange={(event) => field("notifyMembers", event.target.checked)}
                />
                <span>Prévenir les membres</span>
              </label>
            )}
          </fieldset>
        )}
        {!postId && (
          <PublicationSignatureChoice
            ...
            onChange={(event) => field("signatureType", event.target.value)}
          />
        )}
        <button className="btn btn-primary" disabled={busy}>
          {busy ? "Envoi…" : postId ? "Enregistrer les modifications" : "Ouvrir la discussion"}
        </button>
      </form>
    </SafePlaceShell>
  );
