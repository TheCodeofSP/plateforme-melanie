...
export default function AdminAccountsPage() {
  ...
  return (
    <main className="admin-dashboard">
      ...
      <header>
        <div>
          <p className="section-eyebrow">Accès à la plateforme</p>
          <h1>Comptes</h1>
          <p>Retrouvez les membres, intervenantes et administratrices.</p>
        </div>
      </header>
      <section className="admin-filters">
        ...
        <label>
          ...
          >
            ...
            <option value="INTERVENANT">Intervenante</option>
        ...
        <label>
          ...
          <select
            value={accountStatus}
            onChange={(event) => change("statut", event.target.value)}
          >
            <option value="">Tous</option>
      ...
      {error && (
        <ErrorState
          title="Les comptes sont indisponibles"
      ...
      <div className="admin-data-list">
        {data?.users.map((user) => (
          <article key={user._id}>
            ...
            <Link to={`/administration/comptes/${user._id}`}>Consulter</Link>
          </article>
        ))}
      </div>
    </main>
